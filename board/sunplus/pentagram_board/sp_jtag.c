...
void setup_pinmux(unsigned int chip)
{
	...
	if (chip == CHIP_A) {
		...
		printf("reg 0x%08x set as 0x%08x\n", &MOON1_REG_B->sft_cfg[1], MOON1_REG_B->sft_cfg[1]);
		/* 83.20 Configure debug control of CPU (cpu debug ctrl) */
		CFGDBG_REG_B->cfg_dbg_ctrl = (0x01f);
		printf("reg 0x%08x set as 0x%08x\n", &CFGDBG_REG_B->cfg_dbg_ctrl, CFGDBG_REG_B->cfg_dbg_ctrl);
	} else if (chip == CHIP_B) {
		...
		printf("reg 0x%08x set as 0x%08x\n", &MOON1_REG_B->sft_cfg[3], MOON1_REG_B->sft_cfg[3]);
		MOON1_REG_B->sft_cfg[4] = RF_MASK_V_SET(0x1);
		printf("reg 0x%08x set as 0x%08x\n", &MOON1_REG_B->sft_cfg[4], MOON1_REG_B->sft_cfg[4]);
		/* 83.21 Configure debug control of A926 (a926 debug ctrl) */
		CFGDBG_REG_B->cfg_dbg_ctrl = (0x01 | (0x01 << 4));
		printf("reg 0x%08x set as 0x%08x\n", &CFGDBG_REG_B->cfg_dbg_ctrl, CFGDBG_REG_B->cfg_dbg_ctrl);
	}
