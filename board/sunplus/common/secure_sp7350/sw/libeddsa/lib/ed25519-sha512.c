		  ...
		  const uint8_t sk[ED25519_KEY_LEN])
{
	...
	sha3_512_init(&hash);
	sha3_update(&hash, sk, ED25519_KEY_LEN);
	sha3_final(&hash, out);
#endif
...
#else
	sha3_512_init(&hash);
	sha3_update(&hash, sig, 32);
	sha3_update(&hash, pub, 32);
	sha3_update(&hash, data, len);
	sha3_final(&hash, h);
#endif
...
void ed25519_hash(const uint8_t sig[ED25519_SIG_LEN], const uint8_t *data, size_t len,
		const uint8_t pub[ED25519_KEY_LEN], uint8_t h[SHA512_HASH_LENGTH])
{
#ifdef CONFIG_USE_SHA2
	struct sha512 hash;
#else
...
/*
 * Given ed25519 hashed value, verify its signature.
 * Return true if success
 */
bool ed25519_verify_hash(const uint8_t sig[ED25519_SIG_LEN],
	       const uint8_t pub[ED25519_KEY_LEN],
	       const uint8_t hval[SHA512_HASH_LENGTH])
{
	struct ed A, C;
	sc_t t, S;
	uint8_t check[32];
