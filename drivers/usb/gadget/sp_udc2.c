...
static void uphy_init(int port_num)
{
	...
	if (port_num == 0) {
		/* enable clock for UPHY and USBC */
		writel(RF_MASK_V_SET(1 << 13), moon0_reg + CLOCK_ENABLE2);
		writel(RF_MASK_V_SET(1 << 10), moon0_reg + CLOCK_ENABLE2);
		mdelay(1);

		/* reset UPHY0 */
		/* UPHY0_RESET : 1->0 */
		writel(RF_MASK_V_SET(1 << 13), moon0_reg + HARDWARE_RESET2);
		writel(RF_MASK_V_CLR(1 << 13), moon0_reg + HARDWARE_RESET2);
		mdelay(1);

		/* Default value modification */
		writel(0x18888102, uphy_reg + GLOBAL_CONTROL0);

		/* PLL power off/on twice */
		writel(0x88, uphy_reg + PLL_PWR_CTRL_OFFSET);
		mdelay(1);
		writel(0x80, uphy_reg + PLL_PWR_CTRL_OFFSET);
		mdelay(1);
		writel(0x88, uphy_reg + PLL_PWR_CTRL_OFFSET);
		mdelay(1);
		writel(0x80, uphy_reg + PLL_PWR_CTRL_OFFSET);
		mdelay(20);
		writel(0x0, uphy_reg + PLL_PWR_CTRL_OFFSET);

		/* USBC 0 reset */
		/* USBC0_RESET : 1->0 */
		writel(RF_MASK_V_SET(1 << 10), moon0_reg + HARDWARE_RESET2);
		mdelay(1);
		writel(RF_MASK_V_CLR(1 << 10), moon0_reg + HARDWARE_RESET2);
		mdelay(1);
	} else if (port_num == 1) {
		/* enable clock for UPHY and USBC */
		writel(RF_MASK_V_SET(1 << 14), moon0_reg + CLOCK_ENABLE2);
		writel(RF_MASK_V_SET(1 << 11), moon0_reg + CLOCK_ENABLE2);
		mdelay(1);

		/* reset UPHY1 */
		/* UPHY1_RESET : 1->0 */
		writel(RF_MASK_V_SET(1 << 14), moon0_reg + HARDWARE_RESET2);
		writel(RF_MASK_V_CLR(1 << 14), moon0_reg + HARDWARE_RESET2);
		mdelay(1);
		...
		/* USBC 1 reset */
		/* USBC1_RESET : 1->0 */
		writel(RF_MASK_V_SET(1 << 11), moon0_reg + HARDWARE_RESET2);
		mdelay(1);
		writel(RF_MASK_V_CLR(1 << 11), moon0_reg + HARDWARE_RESET2);
		mdelay(1);
	}

	/* HW workaround */
	val = readl(uphy_reg + UPHY_CONFIGS19);
	set = val | 0x0f;
	writel(set, uphy_reg + UPHY_CONFIGS19);

	/* USB DISC (disconnect voltage) */
	writel(0x8b, uphy_reg + DISC_LEVEL_OFFSET);

	/* RX SQUELCH LEVEL */
	writel(0x4, uphy_reg + SQ_CT_CTRL_OFFSET);
}
...
static void usb_power_init(int is_host, int port_num)
{
	...
	if (port_num == 0) {
		if (is_host) {
			writel(RF_MASK_V_SET(7 << 4), moon5_reg + M5_CONFIGS17);
		} else {
			writel(RF_MASK_V_SET(1 << 4), moon5_reg + M5_CONFIGS17);
			writel(RF_MASK_V_CLR(3 << 5), moon5_reg + M5_CONFIGS17);
		}
	} else if (port_num == 1) {
		if (is_host) {
			writel(RF_MASK_V_SET(7 << 12), moon5_reg + M5_CONFIGS17);
		} else {
			writel(RF_MASK_V_SET(1 << 12), moon5_reg + M5_CONFIGS17);
			writel(RF_MASK_V_CLR(3 << 13), moon5_reg + M5_CONFIGS17);
		}
	}
}
...
static void handle_event(struct sp_udc *udc)
{
	...
	aligned_len = roundup(sizeof(struct udc_ring), ARCH_DMA_MINALIGN);
	invalidate_dcache_range((unsigned long)&udc->event_ring[temp_event_sg_count],
					(unsigned long)&udc->event_ring[temp_event_sg_count] + aligned_len);
	...
	invalidate_dcache_range((unsigned long)event_ring_dq, (unsigned long)event_ring_dq + aligned_len);
...
int32_t hal_udc_init(struct sp_udc *udc)
{
	...
	udc->event_seg_table = (struct segment_table *)udc_malloc_align(&udc->event_seg_table_pa,
							udc->event_ring_seg_total * ENTRY_SIZE, ALIGN_64_BYTE, 1);
	...
	for (seg_count = 0; seg_count < udc->event_ring_seg_total; seg_count++) {
		...
		UDC_LOGD("Event_ring[%d]:%px,0x%x -> %px,0x%x\n", seg_count, udc->event_ring[seg_count].trb_va,
						udc->event_ring[seg_count].trb_pa, tmp_ring->trb_va, tmp_ring->trb_pa);
	}

	/* malloc ep description */
	udc->ep_desc = (struct sp_desc *)udc_malloc_align(&udc->ep_desc_pa, ENTRY_SIZE * UDC_MAX_ENDPOINT_NUM,
												ALIGN_32_BYTE, 1);
	if (!udc->ep_desc) {
	...
	UDC_LOGD("device descriptor addr:%px,0x%x\n", udc->ep_desc, udc->ep_desc_pa);
	...
	for (i = 0U; i < UDC_MAX_ENDPOINT_NUM; i++) {
		...
		udc_ring_malloc(udc, &ep->ep_transfer_ring, TRANSFER_RING_COUNT, 1);
					...
					uint8_t ep_type, uint16_t ep_max_packet_size)
{
	...
	ep->ep_trb_ring_dq = ep->ep_transfer_ring.trb_va;
	UDC_LOGD("ep_transfer_ring[%d]:%px,0x%x\n", ep_num, ep->ep_transfer_ring.trb_va,
...
static int sp_udc_probe(struct udevice *udev)
{
	...
	moon5_reg = ioremap(base, sizeof(struct udc_reg));
	if (!udc->reg)
		return -ENOMEM;
	...
	uphy_reg = ioremap(base, sizeof(struct udc_reg));
	if (!udc->reg)
		return -ENOMEM;
...
U_BOOT_DRIVER(sunplus_udc) = {
	...
	.priv_auto_alloc_size = sizeof(struct sp_udc),
};
